  ...
  const handleAdd = async (e) => {
    ...
    try {
      if (editId) {
        ...
        toast.success(`Password for ${website} updated!`);
      } else {
      ...
      setWebsite(''); setUsername(''); setPassword(''); setCategory('Others');
      setEditId(null);
      setShowAdd(false);
    } catch {
      toast.error(editId ? 'Failed to update password' : 'Failed to save password');
    } finally {
      setSaving(false);
    }
  };
        ...
        {showAdd && (
          <div className="add-panel">
            <div className="add-panel-title">
              {editId ? <RiAddLine /> : <RiAddLine />} {editId ? 'Edit Password' : 'Add New Password'}
            </div>
            <form onSubmit={handleAdd}>
              ...
              <div className="add-panel-actions">
                ...
                <button type="submit" className={`btn btn-primary${saving ? ' btn-loading' : ''}`} disabled={saving} style={{ width:'auto' }}>
                  {saving ? '' : <><RiSaveLine size={16} /> {editId ? 'Update Password' : 'Save Password'}</>}
                </button>
              </div>
            </form>
          </div>
